  ...
  useEffect(() => {
    const fetchGamingPosts = async () => {
      try {
        ...
        setGamingProjects(response.data);
      } catch (err) {
        const errorMessage = err.response?.data?.message || 'Failed to fetch gaming posts';
  ...
  return (
    <div className="min-h-screen bg-black">
      <div className="container mx-auto px-4 py-8">
        <h1 className="text-3xl font-bold text-green-500 mb-2 text-center">Gaming Projects</h1>
        <p className="text-gray-400 mb-8 text-center">Join exciting gaming projects</p>
        
        {gamingProjects.length === 0 ? (
          <div className="text-gray-400 text-center py-12">No gaming projects available</div>
        ) : (
